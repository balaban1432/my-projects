...
git clone https://$TOKEN@github.com/balaban1432/private-my-projects.git
...
Go to the S3 console and select the S3 bucket named `awscapstonec3<name>blog`.  # diğer readme de doğrudan lambda menusunden yaptık. bu diğer bir yöntem.
...
```bash
mysql -h Your RDS Endpoint -u admin -p  # parola hardcode olarak -p'nin bitişiğine yazılabilir.
```
